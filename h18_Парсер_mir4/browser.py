...
def create_web(browser, proxy):
    async def event(browser, proxy):
        if browser != '':
        ...
        return [browser, page]
    return asyncio.get_event_loop().run_until_complete(event(browser, proxy))
...
def alt_selector(page, sel):
    async def event(page, sel):
        try:
            return await page.evaluate('(element) => element.alt', sel)
        except Exception as e:
            print(e)
            return None
    return asyncio.get_event_loop().run_until_complete(event(page, sel))
